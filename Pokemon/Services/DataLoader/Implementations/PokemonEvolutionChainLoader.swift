//
//  PokemonEvolutionChainLoader.swift
//  Pokemon
//
...
struct PokemonEvolutionChainLoader: GenericSingleDataLoaderProtocol {
    ...
    func loadDataFromCache(params: Params) throws -> Result<ResultType, Error> {
        ...
        guard let model = try parse(params: DataParseParams_pokemonEvolutionChain(data: data)) else {
    ...
    private func loadOnlineFile(params: Params) async throws -> Params? {
        let loader = LoadFileStrategy_pokemonEvolutionChain()
    ...
    private func parse(params: DataParseParams) throws -> PokemonEvolutionChainModel? {
        switch params {
        case let params as DataParseParams_pokemonEvolutionChain:
            let parser = ParseStrategy_pokemonEvolutionChain()
    ...
    private func saveCacheModel(model: PokemonEvolutionChainModel) {
//        guard let realObj = RealmManager.getPokemon(byID: model.id) else {
//            return
//        }
//        let realm = try! Realm()
//        try! realm.write {
//            realObj.evolutionUrl = model.evolution_chain?.url ?? ""
//            realObj.isSpeciesDataFetched = true
//        }
    }
}

extension PokemonEvolutionChainLoader {
