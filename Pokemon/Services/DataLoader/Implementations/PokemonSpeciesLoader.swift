...
struct PokemonSpeciesLoader: GenericSingleDataLoaderProtocol {
    ...
    func loadDataFromCache(params: Params) throws -> Result<ResultType, Error> {
        ...
        guard let model = try parse(params: DataParseParams_pokemonSpecies(data: data)) else {
    ...
    private func loadOnlineFile(params: Params) async throws -> Params? {
        let loader = LoadFileStrategy_pokemonSpecies()
    ...
    private func parse(params: DataParseParams) throws -> PokemonSpeciesModel? {
        switch params {
        case let params as DataParseParams_pokemonSpecies:
            let parser = ParseStrategy_pokemonSpecies()
...
extension PokemonSpeciesLoader {
